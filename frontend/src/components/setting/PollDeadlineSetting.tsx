...
export const PollDeadlineSetting = (props: { isWeb3: boolean }) => {
  const { t } = useLocale()
  const router = useRouter()
  const { daoId, projectId } = router.query
  const dao = { daoId: daoId as string, projectId: projectId as string }
  //TODO: Buildを通すために一旦isWeb3 = Trueを入れる
  const { pollDetail, loadCurrentMaxPoll, setStartTime, setDuration } = usePoll(dao, props.isWeb3)
  ...
  const onChangeStartDate = (value: Date) => {
    setStartDate(value)
    if (pollDuration && value) setEndDate(new Date(value.getTime() + pollDuration * 24 * 60 * 60 * 1000))
  }

  const setSprintDuration = (value: number | undefined) => {
    setPollDuration(value) //days
    if (value && startDate) setEndDate(new Date(startDate!.getTime() + value * 24 * 60 * 60 * 1000))
  }

  const update = async () => {
    if (pollDetail && startDate && pollDuration) {
      if (pollDetail.startTimeStamp !== startDate) setStartTime(pollDetail.pollId, startDate.getTime() / 1000)
      const duration =
        (pollDetail.endTimeStamp.getTime() - pollDetail.startTimeStamp.getTime()) / (60 * 60 * 24 * 1000 * 1000)
      if (duration !== pollDuration) setDuration(pollDetail.pollId, pollDuration)
    }
  }
  ...
  return (
    <Paper p="lg" mb="lg">
      ...
      <Text size="md">{endDate?.toLocaleDateString()}</Text>
