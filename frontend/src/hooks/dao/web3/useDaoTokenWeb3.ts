...
const useDaoTokenWeb3: useDaoTokenInterface = (props: Props, isWeb3?: boolean) => {
  ...
  usePoll({ daoId: props.daoId, projectId: props.projectId }, isWeb3)
    ...
    if (contract) {
      ...
      if (address) {
        contract.functions
          ...
          .then((b) => setYourBalance(Number(ethers.utils.formatEther(b[0]))))
      }
