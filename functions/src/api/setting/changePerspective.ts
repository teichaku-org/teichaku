...
export const changePerspective = functions.region("asia-northeast1").https.onRequest(async (req, res) => {
  ...
  if (req.method === "OPTIONS") {
    ...
    const requestData: {
      daoId: string
      projectId: string
      perspectiveTexts: string[]
    } = req.body
    const poll = new Poll(requestData.daoId, requestData.projectId, sender)
    await poll.changePerspective(requestData.perspectiveTexts)
